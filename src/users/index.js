...
import { usersData, currentUser, isDirecteur, hashStr,
         ...
         loadAllProfiles, toggleProfileActive, changePassword } from '../auth/index.js';
      ...
      <div class="modal-field"><label>Mot de passe *</label>
        <div style="position:relative;display:flex;align-items:center;">
          <input type="password" id="uf-pw" placeholder="••••••••"
            style="width:100%;padding:9px;border-radius:8px;border:1.5px solid var(--border);font-size:13px;box-sizing:border-box;">
          <button onclick="window.togglePwVis('uf-pw',this)" style="position:absolute;right:8px;background:none;border:none;cursor:pointer;">👁️</button>
        </div></div>
      <div class="modal-field"><label>Confirmer *</label>
        <div style="position:relative;display:flex;align-items:center;">
          <input type="password" id="uf-pw2" placeholder="••••••••"
            style="width:100%;padding:9px;border-radius:8px;border:1.5px solid var(--border);font-size:13px;box-sizing:border-box;">
          <button onclick="window.togglePwVis('uf-pw2',this)" style="position:absolute;right:8px;background:none;border:none;cursor:pointer;">👁️</button>
        </div></div>`:''}
...
export async function saveUserFromModal(userId, getAllRoles) {
  ...
  const sysRole=document.getElementById('uf-sysrole')?.value||'chef_rayon';
  ...
  if(!userId) {
    ...
    if(pw.length<4){showErr('Mot de passe trop court (4 caractères min)');return;}
    const btn=document.getElementById('modal-save-btn');
    if(btn){btn.disabled=true;btn.textContent='⏳ Création...';}
    try {
      if(!_sbClient) throw new Error('Non connecté à Supabase');
      const { data:{ session } } = await _sbClient.auth.getSession();
      if(!session?.access_token) throw new Error('Session expirée');
      const supabaseUrl='https://qpszmqkbhrqlycgdkgop.supabase.co';
      ...
      if(profiles.length>0){ const {setUsersData}=await import('../auth/index.js'); setUsersData(profiles); }
      window.closeModal&&window.closeModal(); renderUsersList();
    ...
    const {error}=await _sbClient.from('hm_profiles').update({
      name, system_role:sysRole, own_role_id:ownRoleId||null, evaluable_role_ids:evalIds
    }).eq('id',userId);
...
export async function resetUserPw(userId) {
  ...
  window._modalSaveCallback=async()=>{
    ...
    } catch(e){const errEl=document.getElementById('rpw-error');if(errEl){errEl.textContent='❌ '+e.message;errEl.style.display='block';}}
  };
  document.getElementById('modal-overlay')?.classList.add('open');
}
...
export async function openChangePwModal() {
  ...
  window._modalSaveCallback=async()=>{
    ...
    const showErr=(m)=>{if(errEl){errEl.textContent=m;errEl.style.display='block';}};
    if(!p1){showErr('Nouveau mot de passe requis');return;}
    if(p1!==p2){showErr('Les mots de passe ne correspondent pas');return;}
    try { await changePassword(p1); window.closeModal&&window.closeModal(); window.showToast&&window.showToast('✅ Mot de passe changé','#27ae60'); }
